...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _parsersTemplate = __webpack_require__(15);
	...
	Vue.prototype._initMethods = function () {
	  ...
	  if (methods) {
	    for (var key in methods) {
	      this[key] = _utils.bind(methods[key], this);
	    }
	  }
	};
	...
	Vue.prototype._initData = function () {
	  ...
	  var data = this._data = dataFn ? _utils.isFunction(dataFn) ? dataFn() : dataFn : {};
	...
	Vue.prototype._bindDir = function (descriptor, node, host, scope) {
	  this._directives.push(new _directive2['default'](descriptor, this, node, host, scope));
	};
	...
	Vue.prototype._compile = function (el) {
	  ...
	  if (options.template) {
	    el = _parsersTemplate.stringToFragment(options.template);
	  } else {
	    el = _parsersTemplate.nodeToFragment(el);
	  }
	
	  var linkFn = _compile2['default'](el);
	  var unlinkFn = linkFn(this, el);
	
	  _utils.replace(original, el);
	};
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _parsersText = __webpack_require__(3);
	
	var _parsersDirective = __webpack_require__(4);
	
	var _utils = __webpack_require__(5);
	
	var _directivesIndex = __webpack_require__(6);
	...
	function checkTerminalDirectives(el, attrs) {
	  ...
	  for (var i = 0, j = attrs.length; i < j; i++) {
	    ...
	    if (matched = name.match(dirAttrRE)) {
	      def = _directivesIndex2['default'][matched[1]];
	      if (def && def.terminal) {
	        if (!termDef) {
	          termDef = def;
	          rawName = attr.name;
	...
	function compileDirectives(attrs) {
	  ...
	  while (i--) {
	    ...
	    if (matched = name.match(dirAttrRE)) {
	      ...
	      // 所以这里的做法是欠妥的
	      dirDef = _directivesIndex2['default'][dirName];
	...
	function processTextToken(token) {
	  ...
	  token.descriptor = {
	    name: type,
	    def: _directivesIndex2['default'][type],
	    expression: parsed.expression
	  };
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var vFor = {
	  ...
	  bind: function bind() {
	    ...
	    _utils.replace(this.el, this.end);
	    _utils.before(this.start, this.end);
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _parsersExpression = __webpack_require__(14);
	...
	function Watcher(vm, expOrFn, cb, options) {
	  ...
	  if (isFn) {
	    ...
	    var res = _parsersExpression.parseExpression(expOrFn);
	    this.getter = res.get;
	    this.setter = res.set;
	  }
	  // 这里是 watcher 收集依赖的入口
